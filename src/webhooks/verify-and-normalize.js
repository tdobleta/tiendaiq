...
function sanitizePayload(topic, payload) {
  ...
  if (topic === "products/update") {
    const id = payload?.admin_graphql_api_id || (payload?.id == null ? null : `gid://shopify/Product/${payload.id}`);
    return { product_gid: typeof id === "string" && /^gid:\/\/shopify\/Product\/\d+$/.test(id) ? id : null };
  }
  return { ignored: true };
}
