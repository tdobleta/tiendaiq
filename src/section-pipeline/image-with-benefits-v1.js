...
function adapt({ product, research, seed, occurrence = 1 }) {
  ...
  const generatedIntro = readCopySlot(research, { section_id: "image-with-benefits", occurrence, field: "intro" });
  ...
  instance.blocks.filter((block) => block.type === "benefit").forEach((block, index) => {
    ...
    const generatedHeading = readCopySlot(research, { section_id: "image-with-benefits", occurrence, block_type: "benefit", block_id: block.id, block_index: index, field: "heading" });
    const generatedBody = readCopySlot(research, { section_id: "image-with-benefits", occurrence, block_type: "benefit", block_id: block.id, block_index: index, field: "body" });
