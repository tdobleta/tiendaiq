...
(function () {
  ...
  const estado = {
    ...
    galeriaCat: "popular", // pestaña activa de la galería de secciones premium
    galeriaQ: "", // búsqueda de la galería
    seccionesInstaladas: {}, // { tipo: {editorUrl} | "cargando" }
    error: null
  };
      ...
      <section class="tarjeta">
        ...
        <div class="herramientas">
          ...
          <div class="herramienta">
            ...
            <p>Secciones de tienda listas (video slider y más). Se agregan y editan desde el editor de temas de Shopify.</p>
...
Me llegó en 3 días y funciona tal cual el video."></textarea>
  ...
  // ============================================================
  // SECCIONES PREMIUM (estilo Section Store) — sections Liquid que la app
  // ESCRIBE en el tema del merchant. Toda la EDICIÓN pasa por el editor de
  // temas nativo de Shopify (panel + bloques desde el {% schema %}). Acá solo
  // vive la GALERÍA de descubrimiento + el botón "Instalar en mi tema".
  // ============================================================
  ...
  // Catálogo de secciones instalables (espeja el de secciones.js en el server).
  ...
  const IC_CAT = {
    popular: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 3l2.5 5.3 5.8.6-4.4 3.9 1.3 5.7L12 18.6l-5.2 3.1 1.3-5.7L3.7 9.9l5.8-.6z"/></svg>`,
  ...
  // Secciones CLÁSICAS que viven en la landing del metafield (no en el tema):
  // se agregan arrastrándolas en el editor de página. El Video slider premium
  // NO está acá (es section del tema, se instala desde la pantalla Secciones).
  const SECCIONES_LANDING = [
    { tipo: "videos", nombre: "Videos de producto", desc: "Carrusel de videos (YouTube, Vimeo o MP4)",
      mini: `<div class="section-card__mini section-card__mini--videos"><span></span><span></span><span></span></div>` },
    { tipo: "carrusel", nombre: "Carrusel de imágenes", desc: "Galería deslizable de fotos",
      mini: `<div class="section-card__mini section-card__mini--imgs"><span></span><span></span><span></span><span></span></div>` }
  ];

  // ---- Pantalla "Secciones premium" (galería estilo Section Store) ----
  function seccionesFiltradas() {
    const cat = estado.galeriaCat || "popular";
  ...
  function seccionesGridHTML() {
    ...
    const inst = estado.seccionesInstaladas || {};
    return items.map((c) => {
      const est = inst[c.tipo]; // undefined | "cargando" | {editorUrl}
      let accion;
      if (est && est.editorUrl) {
        accion = `<div class="galsec-card__ok">${ico("check")} Instalada</div>
          <a class="btn btn--fantasma btn--chico" href="${esc(est.editorUrl)}" target="_blank" rel="noopener">Abrir editor de temas ${ico("externo")}</a>`;
      } else if (est === "cargando") {
        accion = `<button class="btn btn--acento btn--chico" disabled>Instalando…</button>`;
      } else {
        accion = `<button class="btn btn--acento btn--chico" data-inst-sec="${c.tipo}">${ico("subir")} Instalar en mi tema</button>`;
      }
      return `<article class="galsec-card">
        <div class="galsec-card__prev">${c.thumb}</div>
        <div class="galsec-card__pie galsec-card__pie--col">
          <div class="galsec-card__info">
            <div class="galsec-card__nombre">${esc(c.nombre)}</div>
            <div class="galsec-card__desc2">${esc(c.desc)}</div>
          </div>
          <div class="galsec-card__acc">${accion}</div>
        </div>
      </article>`;
    }).join("");
  }

  function pintarSeccionesGrid() {
    const g = $("secpage-grid");
    if (g) g.innerHTML = seccionesGridHTML();
  }

  async function instalarSeccionTema(tipo, btn) {
    estado.seccionesInstaladas = estado.seccionesInstaladas || {};
    estado.seccionesInstaladas[tipo] = "cargando";
    pintarSeccionesGrid();
    try {
      const r = await api("/secciones/instalar", { method: "POST", body: { tipo } });
      estado.seccionesInstaladas[tipo] = { editorUrl: r.editorUrl };
      toast("Sección instalada en tu tema. Abrí el editor de temas para agregarla.");
    } catch (e) {
      estado.seccionesInstaladas[tipo] = undefined;
      toast("No se pudo instalar: " + e.message);
    }
    pintarSeccionesGrid();
  }

  async function pantallaSecciones() {
    estado.galeriaCat = estado.galeriaCat || "popular";
    estado.seccionesInstaladas = estado.seccionesInstaladas || {};
    vista.innerHTML = `
      <div class="secpage">
        <div class="secpage__cab">
          <div>
            <h1 class="secpage__tit">Secciones premium</h1>
            <p class="secpage__sub">Secciones de tienda listas. Se instalan en tu tema y se editan desde el editor de temas de Shopify — con todos sus ajustes nativos.</p>
          </div>
          <div class="galsec__buscar secpage__buscar">${IC_BUSCAR}<input type="text" id="secpage-q" placeholder="Buscar secciones" value="${esc(estado.galeriaQ || "")}"></div>
        </div>
        <div class="galsec__tabs secpage__tabs" id="secpage-tabs">
          ${CATS_SECCIONES.map(([id, lab]) => `<button class="galsec__tab ${estado.galeriaCat === id ? "is-sel" : ""}" type="button" data-cat="${id}"><span class="galsec__tab-ic">${IC_CAT[id] || ""}</span><span>${lab}</span></button>`).join("")}
        </div>
        <div class="galsec__grid secpage__grid" id="secpage-grid">${seccionesGridHTML()}</div>
      </div>`;
    ...
    cont.addEventListener("click", (e) => {
      ...
      if (tab) {
        estado.galeriaCat = tab.dataset.cat;
        cont.querySelectorAll(".galsec__tab").forEach((t) => t.classList.toggle("is-sel", t === tab));
        pintarSeccionesGrid();
        return;
      }
      const inst = e.target.closest("[data-inst-sec]");
      if (inst) { instalarSeccionTema(inst.dataset.instSec, inst); return; }
    });
    const q = $("secpage-q");
    if (q) q.addEventListener("input", () => { estado.galeriaQ = q.value; pintarSeccionesGrid(); });
  }
  ...
  function pantallaPreview() {
    ...
    vista.innerHTML = `
      ...
      <div class="constructor">
        <aside class="panel-sections" id="panel-sections">
          ...
          <a class="panel-sections__prem" id="ir-secciones-prem">${ico("chispa")} Secciones premium →</a>
        </aside>
    ...
    { const b = $("ir-secciones-prem"); if (b) b.onclick = () => ir("secciones"); }
