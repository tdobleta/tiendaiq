(function(){
  ...
  function shell(){
    ...
    // PagePilot uses a pointer-driven sortable surface. Remove the native
    // HTML5 draggable flag before binding so Chrome does not steal the gesture
    // from pointermove/pointerup when the row crosses another section.
    root.querySelectorAll('[data-section-drag], [data-section-drag] .se__tree-select').forEach((item)=>item.removeAttribute('draggable'));
    bind();refreshPreview();
  }
  function bind(){
