(function(){
  ...
  function treeHtml(){
    ...
    const entry=definition(section);const active=section.id===state.selectedSection&&!state.selectedBlock&&!state.selectedOutline;const dragEnabled=entry?.capabilities?.protected!==true&&entry?.capabilities?.reorderable!==false&&index>=sectionOrderFloor();const dragAttrs=dragEnabled?` aria-roledescription="sortable" aria-describedby="se-section-drag-help" draggable="true" data-section-drag="${esc(section.id)}"`:` aria-disabled="true"`;
      ...
      return `<div class="se__tree-section"><div class="se__tree-main ${active?"is-active":""}"><button class="se__tree-toggle ${expanded?"is-expanded":""}" type="button" data-expand-section="${esc(section.id)}" aria-expanded="${expanded}" aria-label="${expanded?"Contraer":"Expandir"} ${esc(section.label)}">${treeIcon("chevron")}</button><button class="se__tree-select" type="button" data-section="${esc(section.id)}" aria-pressed="${active}"${dragAttrs}>${treeIcon("section")}<span>${esc(section.label)}</span><small>(${count})</small></button>${rowActions}</div><div class="se__tree-blocks" ${expanded?"":"hidden"}>${contents}</div></div>${sectionInsertSlot(index+1)}`;
    }).join("");
  }
