(function(){
  ...
  function shell(){
    ...
    if(identity){
      identity.insertAdjacentHTML("beforeend",`<button type="button" class="se__advanced-toggle ${state.advanced?"is-on":""}" id="se-advanced" aria-pressed="${state.advanced}" title="Mostrar controles avanzados">${treeIcon("settings")}<span>Modo avanzado</span></button><button type="button" class="se__brand-button" id="se-brand" aria-expanded="false" title="Editar marca"><i></i><i></i><i></i><span>Marca</span></button>`);
    }
    const actions=root.querySelector(".se__actions-popover");
    if(actions)actions.insertAdjacentHTML("beforeend",`<button type="button" id="se-open-preview">Abrir vista previa</button>`);
