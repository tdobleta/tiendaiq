(function(){
  ...
  function syncDirty(){state.dirty=fingerprint(state.page)!==state.savedFingerprint;const saveButton=root.querySelector("#se-save");if(saveButton)saveButton.disabled=!state.dirty;const publishButton=root.querySelector("#se-publish");if(publishButton){publishButton.disabled=demo||state.publishing||(!state.dirty&&state.published);const label=publishButton.querySelector(".se__action-label");if(label)label.textContent=state.publishing?"Publicando…":state.published?"Publicada":"Publicar en la tienda"}const undoButton=root.querySelector("#se-undo");if(undoButton)undoButton.disabled=!state.past.length;const redoButton=root.querySelector("#se-redo");if(redoButton)redoButton.disabled=!state.future.length}
  function changed(){state.published=false;syncDirty()}
  ...
  function sourceIcon(origin){const tone=origin.kind==="shopify"?"success":origin.kind==="ai"?"info":"neutral";return `<s-badge class="se__origin-badge se__origin-badge--${origin.kind}" tone="${tone}" title="${origin.title}" accessibility-label="Origen: ${origin.label}" aria-label="Origen: ${origin.label}">${origin.label}</s-badge>`}
  function fieldHtml(field,settings,scope,locked=false,aiEnabled=false,originKind="template"){
    ...
    const value=fieldValue(field,settings);const disabled=locked?' disabled aria-disabled="true"':'';const attr=`data-field="${esc(field.id)}" data-scope="${scope}" aria-label="${esc(field.label||field.id)}"${locked?' data-field-locked="true"':''}`;const placeholder=originKind==="shopify"?"Usa el valor actual de Shopify":(field.placeholder||"");
  ...
  function treeIcon(type){
    ...
    const paths={
      ...
      publish:'<path d="M8 11V3M5 6l3-3 3 3M3 10.5v2h10v-2"/>',
      ...
      back:'<path d="M10.5 3.5 6 8l4.5 4.5M6 8h7"/>',
      undo:'<path d="M6 5 3 8l3 3M3.5 8H9a4 4 0 1 1 0 4"/>',
      redo:'<path d="m10 5 3 3-3 3M12.5 8H7a4 4 0 1 0 0 4"/>',
  ...
  function sectionMenuHtml(section){const open=state.sectionMenuId===section.id;return `<div class="se__section-menu" data-section-menu-popover="${esc(section.id)}" role="menu" ${open?"":"hidden"}><s-button variant="tertiary" role="menuitem" data-section-action="select" data-section-id="${esc(section.id)}">Seleccionar</s-button><s-button variant="tertiary" role="menuitem" data-section-action="save" data-section-id="${esc(section.id)}" ${state.dirty?"":"disabled"}>Guardar cambios</s-button><s-button variant="tertiary" role="menuitem" data-section-action="duplicate" data-section-id="${esc(section.id)}">Duplicar sección</s-button><s-button variant="tertiary" tone="critical" role="menuitem" data-section-action="delete" data-section-id="${esc(section.id)}">Eliminar sección</s-button></div>`}
  function treeHtml(){
    ...
    const entry=definition(section);const active=section.id===state.selectedSection&&!state.selectedBlock&&!state.selectedOutline;const dragEnabled=entry?.capabilities?.protected!==true&&entry?.capabilities?.reorderable!==false&&index>=sectionOrderFloor();const dragAttrs=dragEnabled?` aria-roledescription="sortable" aria-describedby="se-section-drag-help" data-section-drag="${esc(section.id)}"`:` aria-disabled="true"`;
      ...
      const rowActions=dragEnabled?`<div class="se__tree-actions"><s-button variant="tertiary" data-section-preview="${esc(section.id)}" accessibility-label="Vista previa de ${esc(section.label)}" aria-label="Vista previa de ${esc(section.label)}" title="Vista previa">${treeIcon("eye")}</s-button><s-button variant="tertiary" data-section-menu="${esc(section.id)}" accessibility-label="Más opciones de ${esc(section.label)}" aria-label="Más opciones de ${esc(section.label)}" title="Más opciones">${treeIcon("more")}</s-button>${sectionMenuHtml(section)}</div>`:"";
  ...
  function syncAdvancedVisibility(){root.querySelectorAll(".se__group").forEach((group)=>{const title=group.querySelector("h3")?.textContent||"";if(/avanzad|advanced/i.test(title))group.hidden=!state.advanced})}
  function bindPreviewInteractions(frame){const doc=frame?.contentDocument;if(!doc||doc.documentElement.dataset.tiqPreviewInteractions==="1")return;doc.documentElement.dataset.tiqPreviewInteractions="1";const activateTab=(button)=>{const tabs=button.closest("[data-tabs]")||button.closest("[id^=product-hero-]");if(!tabs)return;const buttons=[...tabs.querySelectorAll("[data-tab-button]")];const panels=[...tabs.querySelectorAll("[data-tab-panel]")];const activeClass=[...button.classList].find((name)=>name.endsWith("__tab"))?.replace(/__tab$/,"__tab--active");buttons.forEach((item)=>{const active=item===button;if(activeClass)item.classList.toggle(activeClass,active);item.setAttribute("aria-selected",active?"true":"false");item.tabIndex=active?0:-1});panels.forEach((panel)=>{const active=panel.id===button.dataset.panelId;const visibleClass=[...panel.classList].find((name)=>name.endsWith("__tab-panel"))?.replace(/__tab-panel$/,"__tab-panel--visible");panel.hidden=!active;if(visibleClass)panel.classList.toggle(visibleClass,active)})};doc.addEventListener("click",(event)=>{const tab=event.target.closest?.("[data-tab-button]");if(tab){activateTab(tab);return}const bundle=event.target.closest?.("[data-bundle]");if(bundle){const group=bundle.closest("[role=group]")||bundle.parentElement;group?.querySelectorAll("[data-bundle]").forEach((item)=>{const active=item===bundle;item.setAttribute("aria-pressed",active?"true":"false");item.dataset.selected=active?"true":"false"});}});doc.addEventListener("keydown",(event)=>{const tab=event.target.closest?.("[data-tab-button]");if(!tab||!["ArrowRight","ArrowLeft","Home","End"].includes(event.key))return;const tabs=tab.closest("[data-tabs]")||tab.closest("[id^=product-hero-]");const buttons=[...(tabs?.querySelectorAll("[data-tab-button]")||[])];const index=buttons.indexOf(tab);if(index<0)return;let next=index;if(event.key==="ArrowRight")next=(index+1)%buttons.length;if(event.key==="ArrowLeft")next=(index-1+buttons.length)%buttons.length;if(event.key==="Home")next=0;if(event.key==="End")next=buttons.length-1;event.preventDefault();activateTab(buttons[next]);buttons[next].focus()})}
  ...
  function bindDynamicTreeInteractions(){
    ...
    root.querySelectorAll('[data-insert-kind="section"]').forEach((slot)=>slot.querySelector("s-button")?.addEventListener("click",()=>openSectionLibrary(Number(slot.dataset.insertIndex))));
  }
  ...
  function openSectionLibrary(index=state.page.sections.length){const nextIndex=Number.isInteger(index)?Math.max(sectionOrderFloor(),Math.min(index,state.page.sections.length)):state.page.sections.length;state.insertTarget={kind:"section",index:nextIndex};state.libraryOpen=true;state.libraryCategory="Todas";shell();requestAnimationFrame(()=>root.querySelector("[data-library-search]")?.focus())}
  ...
  function ensurePublishButtonIcon(){const button=root.querySelector("#se-publish");if(!button||button.querySelector("svg"))return;const label=button.textContent.trim()||"Publicar en la tienda";button.innerHTML=`${treeIcon("publish")}<span class="se__action-label">${esc(label)}</span>`}
  function shell(){
    // Contrato del control de publicación: <s-button variant="primary" id="se-publish"/>.
    root.innerHTML=`<div class="se ${state.fullPreview?"is-full-preview":""}"><header class="se__top"><div class="se__identity"><s-button variant="tertiary" class="se__back" id="se-back" accessibility-label="Volver" title="Volver">${treeIcon("back")}</s-button><span class="se__brand-mark" aria-hidden="true">✦</span><div class="se__title"><b>${esc(state.pageTitle)}</b><small>Editor por secciones</small></div></div><div class="se__viewport" role="group" aria-label="Herramientas de vista"><s-button variant="tertiary" class="se__viewport-tool ${state.fullPreview?"":"is-selected"}" id="se-select" accessibility-label="Seleccionar" title="Seleccionar" aria-pressed="${!state.fullPreview}">${treeIcon("select")}</s-button><s-button variant="tertiary" class="se__viewport-tool" id="se-desktop" accessibility-label="Vista de escritorio" title="Vista de escritorio" aria-pressed="${!state.mobile}">${treeIcon("desktop")}</s-button><s-button variant="tertiary" class="se__viewport-tool" id="se-mobile" accessibility-label="Vista móvil" title="Vista móvil" aria-pressed="${state.mobile}">${treeIcon("mobile")}</s-button><s-button variant="tertiary" class="se__viewport-tool" id="se-fullscreen" accessibility-label="Vista previa" title="Vista previa" aria-pressed="${state.fullPreview}">${treeIcon("fullscreen")}</s-button></div><div class="se__actions"><s-button variant="tertiary" class="se__history" id="se-undo" accessibility-label="Deshacer" title="Deshacer" ${state.past.length?"":"disabled"}>${treeIcon("undo")}</s-button><s-button variant="tertiary" class="se__history" id="se-redo" accessibility-label="Rehacer" title="Rehacer" ${state.future.length?"":"disabled"}>${treeIcon("redo")}</s-button><s-button variant="secondary" class="se__action-button se__action-button--secondary" id="se-save" ${state.dirty?"":"disabled"}>Guardar</s-button><s-button variant="primary" class="se__action-button se__action-button--primary" id="se-publish" ${demo?"disabled":""}>Publicar en la tienda</s-button><s-button variant="secondary" class="se__action-button se__action-button--secondary" id="se-variants" ${productNumericId()?"":"disabled"}>Editar variantes</s-button><div class="se__actions-menu"><s-button variant="secondary" class="se__action-button se__action-button--secondary" id="se-actions" aria-expanded="false">${treeIcon("settings")}<span>Acciones</span></s-button><div class="se__actions-popover" hidden><s-button variant="tertiary" id="se-expand-all">Expandir todos los elementos</s-button><s-button variant="tertiary" id="se-collapse-all">Contraer todos los elementos</s-button></div></div></div></header><div class="se__body"><aside class="se__panel se__panel--left"><div class="se__panel-head"><b>Página de producto</b><span class="se__drag-help" id="se-section-drag-help">Mantené pulsada una sección para reordenarla. Con foco: espacio, flechas y espacio.</span></div><nav class="se__tree" aria-label="Secciones de la página">${treeHtml()}</nav><s-button variant="tertiary" class="se__add" id="se-add" ${availableSections().length?"":"disabled"}><span aria-hidden="true">${treeIcon("plus")}</span>Añadir sección</s-button></aside><section class="se__canvas"><div class="se__frame-shell ${state.mobile?"is-mobile":""}"><iframe class="se__frame" id="se-frame" title="Vista previa de la página"></iframe></div></section><aside class="se__panel se__panel--right"><div class="se__inspector">${inspectorHtml()}</div></aside></div></div>${libraryHtml()}`;
    ensurePublishButtonIcon();
    const identity=root.querySelector(".se__identity");
    if(identity){
      identity.insertAdjacentHTML("beforeend",`<s-button variant="secondary" class="se__advanced-toggle ${state.advanced?"is-on":""}" id="se-advanced" aria-pressed="${state.advanced}" title="Mostrar controles avanzados">${treeIcon("settings")}<span>Modo avanzado</span></s-button><s-button variant="secondary" class="se__brand-button" id="se-brand" aria-expanded="false" title="Editar marca"><i></i><i></i><i></i><span>Marca</span></s-button>`);
    }
    const actions=root.querySelector(".se__actions-popover");
    if(actions)actions.insertAdjacentHTML("beforeend",`<s-button variant="tertiary" id="se-open-preview">Abrir vista previa</s-button>`);
  ...
  function bind(){
    ...
    root.querySelector("#se-advanced")?.addEventListener("click",()=>{state.advanced=!state.advanced;root.querySelector(".se")?.classList.toggle("is-advanced",state.advanced);root.querySelector("#se-advanced").classList.toggle("is-on",state.advanced);root.querySelector("#se-advanced").setAttribute("aria-pressed",String(state.advanced));syncAdvancedVisibility()});
    root.querySelector("#se-brand")?.addEventListener("click",(event)=>{event.stopPropagation();const current=root.querySelector("#se-brand").getAttribute("aria-expanded")==="true";root.querySelector("#se-brand").setAttribute("aria-expanded",String(!current));notify(current?"Panel de marca cerrado.":"La marca se administra desde la configuración de la tienda.")});
    ...
    root.querySelector("#se-open-preview")?.addEventListener("click",()=>{const frame=root.querySelector("#se-frame");if(!frame?.srcdoc)return notify("La vista previa todavía se está cargando.");const url=URL.createObjectURL(new Blob([frame.srcdoc],{type:"text/html"}));window.open(url,"_blank","noopener,noreferrer");setTimeout(()=>URL.revokeObjectURL(url),60000)});
  ...
  async function refreshPreview(){const request=++state.previewRequest;state.previewAbort?.abort();const controller=new AbortController();state.previewAbort=controller;try{const path=demo?"/section-page-demo-preview":`/api/paginas/${encodeURIComponent(pageId)}/section-preview`;const result=await api(path,{method:"POST",signal:controller.signal,body:{section_page:state.page}});if(request!==state.previewRequest||controller.signal.aborted)return;const frame=root.querySelector("#se-frame");if(frame){const next=frame.cloneNode(false);next.removeAttribute("src");next.classList.add("se__frame--pending");next.onload=()=>{if(request!==state.previewRequest||controller.signal.aborted){next.remove();return}next.className=frame.className;frame.replaceWith(next);bindPreviewInteractions(next);state.previewAbort=null;focusPreview()};frame.parentElement?.appendChild(next);next.srcdoc=result.html}}catch(error){if(error.name!=="AbortError"&&request===state.previewRequest)notify(error.message)}}
  ...
  async function publish(){const button=root.querySelector("#se-publish");if(!button||state.publishing||demo)return;state.publishing=true;syncDirty();try{if(state.dirty)await persist({render:false,announce:false});const{job}=await api(`/api/paginas/${encodeURIComponent(pageId)}/publicar`,{method:"POST"});const completed=await waitForJob(job.id);state.publishing=false;state.published=true;syncDirty();notify("Página publicada en Shopify.");if(completed.result?.url)window.open(completed.result.url,"_blank","noopener")}catch(error){state.publishing=false;syncDirty();notify(error.message)}}
  window.addEventListener("keydown",(event)=>{const editing=event.target?.matches?.("input,textarea,select,[contenteditable=true]");if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==="s"){event.preventDefault();if(state.dirty)save();return}if(!editing&&(event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==="z"){event.preventDefault();if(event.shiftKey)redo();else undo();return}if(!editing&&(event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==="y"){event.preventDefault();redo();return}if(event.key==="Escape"){const ai=root.querySelector("[data-ai-modal]");if(ai){ai.remove();return}if(root.querySelector(".se__media-backdrop[data-media-close]")){closeGallery();return}if(state.libraryOpen){state.libraryOpen=false;state.insertTarget=null;shell()}}});
  ...
  async function start(){if(demo){const sample=await api("/section-page-demo-data");state.page=clone(sample.section_page);state.language=sample.idioma||"es";state.pageTitle=`${sample.titulo} · demostración`;state.registry=sample.registry||[]}else{if(!pageId)throw new Error("Falta el id de la página.");const[page,registry]=await Promise.all([api(`/api/paginas/${encodeURIComponent(pageId)}`),api("/api/section-registry")]);if(!page.data?.section_page)throw new Error("Esta página todavía no usa el sistema por secciones.");state.page=clone(page.data.section_page);state.language=page.data?.global?.idioma||"es";state.pageTitle=page.titulo||state.page.productSnapshot?.title||"Página de producto";state.registry=registry.sections||[]}state.savedFingerprint=fingerprint(state.page);state.published=false;state.publishing=false;state.selectedSection=state.page.sections[0]?.id||null;state.selectedBlock=null;state.selectedOutline=null;state.expandedSections.clear();state.expandedOutline.clear();const first=state.page.sections[0];if(first){state.expandedSections.add(first.id);expandAllOutline(first,definition(first)?.editor.outline)}status.hidden=true;root.hidden=false;shell()}
