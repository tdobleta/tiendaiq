...
test("las secciones se pueden reordenar sobre el mismo modelo y con teclado", () => {
  ...
  assert.match(source, /targetIndex=pointerSectionTarget\(event\);state\.pointerDrag=null;if\(targetIndex===null\)\{clearSectionDrag\(\);return\}/);
  assert.match(source, /targetIndex=pointerSectionTarget\(event\);state\.pointerDrag=null/);
