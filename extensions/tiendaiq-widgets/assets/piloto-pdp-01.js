(() => {
  ...
  const money = (variant, quantity) => variant?.money ? variant.money : "";
  const packs = (c.offer?.packs || []).map((pack) => ({ ...pack, variant: variantFor(pack) })).filter((pack) => pack.variant);
  let active = packs.find((pack) => pack.variant.available) || packs[0] || null;
